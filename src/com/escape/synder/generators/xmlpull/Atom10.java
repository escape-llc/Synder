...
public final class Atom10 extends XmlPullGeneratorImpl {
	...
	public <T> void generate(SyndFeed sf, String enc, T outputMedium)
		...
		xs.setPrefix(defaultns, com.escape.synder.handlers.Atom10.ATOM_10_URI);
		...
		try {
			xs.startTag(com.escape.synder.handlers.Atom10.ATOM_10_URI, "feed");
			try {
				xs.attribute(null, "version", "1.0");
				if(sf.getTitleEx() != null) {
					contentElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "title", sf.getTitleEx());
				}
				if(sf.getDescriptionEx() != null) {
					contentElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "subtitle", sf.getDescriptionEx());
				}
				if(sf.getGenerator() != null) {
					generatorElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, sf.getGenerator());
				}
				for(final SyndPerson sp: sf.getAuthors()) {
					personElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "author", sp);
				}
				for(final SyndPerson sp: sf.getContributors()) {
					personElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "contributor", sp);
				}
				for (final SyndLink sl : sf.getLinks()) {
					linkElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, sl);
				}
				final SyndImage img = sf.getImage();
				if(img != null) {
					textElementOptional(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "icon", img.getUrl());
				}
				final SyndImage img2 = sf.getLogo();
				if(img2 != null) {
					textElementOptional(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "logo", img2.getUrl());
				}
				final SyndContent rights = sf.getRights();
				if(rights != null) {
					contentElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "rights", rights);
				}
				applyModules(xs, sf.getModules(), fns);
				xs.ignorableWhitespace("\n");
				for (final SyndEntry se : sf.getEntries()) {
					xs.startTag(com.escape.synder.handlers.Atom10.ATOM_10_URI, "entry");
					try {
						textElementOptional(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "id", se.getLink());
						if(se.getTitleEx() != null) {
							contentElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "title", se.getTitleEx());
						}
						dateElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "published", se.getPublishedDate(), ctx.getDateFormatW3C());
						dateElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "updated", se.getUpdatedDate(), ctx.getDateFormatW3C());
						final SyndContent erights = se.getRights();
						if(erights != null) {
							contentElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "rights", erights);
						}
						...
						for(final SyndCategory sc: se.getCategories()) {
							categoryElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, sc);
						}
						for(final SyndEnclosure senc: se.getEnclosures()) {
							enclosureElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, senc);
						}
						applyModules(xs, se.getModules(), ins);
						xs.ignorableWhitespace("\n");
						for (final SyndContent sc : se.getContents()) {
							contentElement(xs, com.escape.synder.handlers.Atom10.ATOM_10_URI, "content", sc);
							xs.ignorableWhitespace("\n");
						}
					} finally {
						xs.endTag(com.escape.synder.handlers.Atom10.ATOM_10_URI, "entry");
						xs.ignorableWhitespace("\n");
					}
				}
			} finally {
				xs.endTag(com.escape.synder.handlers.Atom10.ATOM_10_URI, "feed");
			}
		} finally {
			xs.endDocument();
		}
